...
bool PositionSecondOrderStep2::time_acc0(Profile& profile, double vMax, double vMin, double aMax, double aMin) {
    // UD Solution 1/2
    {
        const double h1 = compat_sqrt((2*aMax*(pd - tf*vf) - 2*aMin*(pd - tf*v0) + vd*vd)/(aMax*aMin) + tf*tf);
...
bool PositionSecondOrderStep2::time_none(Profile& profile, double vMax, double vMin, double aMax, double aMin) {
    if (compat_abs(v0) < DBL_EPSILON && compat_abs(vf) < DBL_EPSILON && compat_abs(pd) < DBL_EPSILON) {
        profile.t[0] = 0;
        profile.t[1] = tf;
        profile.t[2] = 0;
        profile.t[3] = 0;
